...
export default function KadrajinOtesiIndexPage() {
  ...
  return <main className="exhibition-page beyond-page"><ExhibitionHeader />
    ...
    <section className="beyond-posts" aria-label="Yazılar">{posts.map((post, index) => <article key={post.slug} className="reveal-up"><span>{post.dateLabel}</span><Link href={`/kadrajin-otesi/${post.slug}`}><small>{String(index + 1).padStart(2,"0")}</small><h2>{post.title}</h2><p>{post.description}</p><b>Okumaya geç →</b></Link></article>)}</section>
    <ExhibitionFooter />
  </main>;
}
