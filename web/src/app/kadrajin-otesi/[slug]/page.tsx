...
export default async function BeyondArticle({params}:{params:Promise<{slug:string}>}) {
  ...
  return <main className="exhibition-page beyond-article"><ExhibitionHeader /><article><Link href="/kadrajin-otesi" className="beyond-back">← Kadrajın Ötesinde</Link><header><span>{post.dateLabel}</span><h1>{post.title}</h1><p>{post.description}</p></header><KadrajinOtesiReading paragraphs={body} /></article><aside className="beyond-article__end"><span aria-hidden>✨</span><p>Bazı cümleler,<br /><em>fotoğraftan sonra başlar.</em></p></aside><ExhibitionFooter /></main>;
}
